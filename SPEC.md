...
- `LLMProvider.swift` — `LLMStreamingProvider` protocol, `ClaudeSSEProvider` (SSE over `URLSession.AsyncBytes`, no SDK), `OnDeviceFallbackProvider` (Foundation Models), and `LLMProviderFactory`. Covers streaming SSE parsing, API key resolution precedence, request encoding, transparent failover, and refusal handling. Wired into `ThreadOrchestrator` through `LLMProviderFactory`.
...
- `ThreadOrchestrator.swift` — Layer 4, the full message lifecycle through a single `send()` entry point. A `ModelActor` whose `ModelContext` is confined to its own serial `ModelExecutor` (created from the shared `ModelContainer`, separate from the UI's context), so its work runs genuinely off the main thread rather than merely interleaving with it, and the context is used only on the executor it is bound to. `ModelActor` conformance is written by hand rather than via the `@ModelActor` macro so the dependency-injecting `init`/`makeDefault` survive. `send()` streams steps 1-7 (save, embed, retrieve, assemble, stream via `LLMProviderFactory`, save) to the caller, then continues steps 8-12 (extract, escalate low-confidence items to Claude, mark superseded nodes, embed new nodes, summarize every tenth message) in a detached background `Task` the caller never awaits. Prompt/formatting logic (`OrchestrationPrompts`, `EscalationPromptBuilder`) is split into pure `nonisolated` enums, and the SwiftData-touching halves of extraction and escalation (`applyExtraction`, `escalate`) are exposed at `internal` access specifically so they are testable with hand-built `CalibratedExtraction` values and a stub `LLMStreamingProvider`, without depending on a working on-device model.
- `AudioStreamManager.swift` — voice capture. A `@MainActor @Observable` controller that requests microphone + speech-recognition permissions, captures audio via `AVAudioEngine`, and streams it through `SpeechAnalyzer`/`SpeechTranscriber` for on-device live transcription, publishing the live transcript and a rolling waveform-level window. Permission resolution, transcript assembly (volatile-replaces / final-appends), and waveform math are pulled into pure `nonisolated` value types so they are unit-testable without a microphone. A release-during-`.preparing` guard stops a start that completes after the finger lifts, and the Speech model is prewarmed on launch so the first hold is responsive. Callbacks that fire off-main (`SFSpeechRecognizer.requestAuthorization`, the `AVAudioEngine` tap) are `nonisolated`/`@Sendable` so Swift 6's executor check doesn't trap under the project's default `MainActor` isolation. The live audio/Speech path is device-only — the simulator has no Speech model.
...
- `AudioStreamManagerTests.swift` — permission resolution, live-transcript assembly, waveform-level math, and workstream titling. All pure `nonisolated` helpers, so these are real assertions rather than skipped hardware tests; the audio/`SpeechAnalyzer` path itself is device-only. 20 tests.

**/App and Views**

- `ThreadsApp.swift` — constructs the shared `ModelContainer` and the default `ThreadOrchestrator` and injects both into the UI.
- `HomeView.swift` — workstream cards over a live voice-capture bar. Hold to record (waveform + live transcript, medium/light haptics per `rules/ui.md`); on release a new workstream is created and its first turn is driven through `ThreadOrchestrator.send()`. Home has no selected workstream and voice routing is out of scope, so a capture deliberately opens a *new* workstream.
- `WorkstreamDetailView.swift` — three-tab detail. **Stream** is the live conversation wired to `send()`; **Context** renders the knowledge graph; **Insights** is stubbed. Long-pressing an AI turn opens the retrieval debug inspector (assembled system prompt + live scores).

### Not yet built

- **Insights tab** — proactive-surfacing feed, deliberately stubbed this cycle (`rules/ui.md`).

**Consequence:** a message now runs end to end from the app — spoken (or typed in the Stream tab) → transcribed on-device → retrieval and context assembly → Claude with on-device fallback → streamed back and persisted, with extraction, escalation, and summary continuing in the background. The live voice path is confirmed on a physical device (no Speech model on the simulator); the surrounding permission, state-transition, and assembly logic is covered by unit tests. The eval scoring finding (below) remains the open work.
